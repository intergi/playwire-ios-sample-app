...
@interface InlineBannerViewController () <PWViewAdDelegate>
@property (weak, nonatomic) IBOutlet UILabel *statusLabel;
@property (strong, nonatomic) PWBannerViewInline *bannerView;
...
- (void)viewDidLoad {
    ...
    self.bannerView = [[PWBannerViewInline alloc] initWithAdUnitName:self.adUnitName delegate:self];
    ...
    //   @"age": @"18-32",
    //   @"page": @"travel"
    // } withWidth:320] withDeviceOrientation:UIInterfaceOrientationUnknown];
    // [self.bannerView loadWithParams:params];
    
    // Take the current orientation of the window scene or we can use `UIInterfaceOrientationUnknown` value to allow SDK to
    // determine the orientation under the hood.
    
    UIInterfaceOrientation orientation = self.view.window.windowScene.interfaceOrientation;

    PWLoadParams *params = [[[PWLoadParams new] withWidth:self.view.frame.size.width] withDeviceOrientation:orientation];
    [self.bannerView loadWithParams:params];
