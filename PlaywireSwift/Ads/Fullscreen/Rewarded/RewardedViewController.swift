...
final class RewardedViewController: UIViewController {
    ...
    private func loadRewarded() {
        ...
        rewarded?.load()
        statusLabel.text = "⏳ The rewarded \"\(adUnitName)\" is loading."
    }
}
...
extension RewardedViewController: PWFullScreenAdDelegate {
    func fullScreenAdDidLoad(_ ad: PWFullScreenAd) {
        statusLabel.text = "✅ The rewarded \"\(adUnitName)\" is loaded."
    ...
    func fullScreenAdDidRecordImpression(_ ad: PWFullScreenAd) {
        statusLabel.text = "👍 The rewarded \"\(adUnitName))\" was successfully shown."
    }
