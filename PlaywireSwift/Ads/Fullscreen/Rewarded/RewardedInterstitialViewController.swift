...
final class RewardedInterstitialViewController: UIViewController {
    ...
    private func loadRewardedInterstitial() {
        ...
        statusLabel.text = "⏳ The rewarded interstitial \"\(adUnitName)\" is loading."
    }
...
extension RewardedInterstitialViewController: PWFullScreenAdDelegate {
    func fullScreenAdDidLoad(_ ad: PWFullScreenAd) {
        statusLabel.text = "✅ The rewarded interstitial \"\(adUnitName)\" is loaded."
        showRewardedInterstitial()
    }
    
    func fullScreenAdDidFailToLoad(_ ad: PWFullScreenAd) {
        statusLabel.text = "❌ Failed to load the rewarded interstitial \"\(adUnitName)\"."
    }
    ...
    func fullScreenAdDidFailToPresentFullScreenContent(_ ad: PWFullScreenAd) {
        statusLabel.text = "❌ Failed to show the rewarded interstitial \"\(adUnitName)\"."
    }
    
    func fullScreenAdDidRecordImpression(_ ad: PWFullScreenAd) {
        statusLabel.text = "👍 The rewarded interstitial \"\(adUnitName))\" was successfully shown."
    }
    ...
    func fullScreenAdDidUserEarn(_ ad: PWFullScreenAd, type: String, amount: Double) {
        ...
        statusLabel.text =  "🎉 The reward was earned.\n Type: \(type)\n Amount: \(String(format: "%0.f", amount))."
    }
}
